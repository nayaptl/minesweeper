    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):
                ...
                # if cell in bounds and is mine then add in newcells
                if 0 <= i < self.height and 0 <= j < self.width:
                    newcells.add((i,j))
        ...
        mine=set()
        safe=set()
        for new in self.knowledge:
            
        
            for cell in new.known_mines():
                mine.add(cell)
            for cell in new.known_safes():
                safe.add(cell)
        for cell in mine:
            self.mark_mine(cell)
        for cell in safe:
            self.mark_safe(cell)
              ...
              # if they can be inferred from existing knowledge
        for s1, s2 in itertools.combinations(self.knowledge, 2):#makes pairs of existing sentences
                newsen= s1.infer_from(s2)
                if newsen is not None and newsen not in self.knowledge:
    ...
    def make_safe_move(self):
        ...
        for cell in self.safes:
            if cell not in self.moves_made:
                if self.countmoves==totalmoves:
                     return None
                print("",self.countmoves)
                self.countmoves+=1
                return cell
